...
class DMGNet(nn.Module):
    def __init__(self, T=4, k=1, img_size=128, embed_dim=32, depths=None,
        ...
        self.gm = nn.ModuleList([GM(embed_dim) for _ in range(T)])
        self.G = Gradient_Guidance()
        self.M = Fusion(embed_dim)
        #self.prior = Prior(embed_dim)

    def forward(self, y, gt):
        x = self.transform_function(y)  # Frequent domain
        ####### Multiple Iterations #######
        for i in range(self.T):
            v = self.dmr(x)
            # Gradient and Feature Fusion
            #z = self.fusion(self.G(x), x)
            # Gating Module
            delta = self.gm[i](z, v)
            x = torch.mul((1 - delta), x) + torch.mul(delta, v + self.fusion(y - self.fusion(self.G(x), x)))
