...
class SIDDData(u_data.Dataset):
    def __init__(self, path, patch_size):
        ...
        clean_files = sorted(os.listdir(os.path.join(path, 'patches', 'GT')))
        noisy_files = sorted(os.listdir(os.path.join(path, 'patches', 'Noisy')))
        # training data
        self.clean_filenames = [os.path.join(path, 'patches', 'GT', x) for x in clean_files if is_png_file(x)]
        self.noisy_filenames = [os.path.join(path, 'patches', 'Noisy', x) for x in noisy_files if is_png_file(x)]
