...
class SIDDData(u_data.Dataset):
    def __init__(self, path, patch_size):
        ...
        clean_files = sorted(os.listdir(os.path.join(path, 'target')))
        noisy_files = sorted(os.listdir(os.path.join(path, 'input')))
        # training data
        self.clean_filenames = [os.path.join(path, 'target', x) for x in clean_files]
        self.noisy_filenames = [os.path.join(path, 'input', x) for x in noisy_files]
